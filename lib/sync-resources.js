...
const TOML_COMMAND_MAP = {
  ...
  'handoff': {
    ...
    prompt: 'Invoke the metaswarm handoff skill. The user wants a session handoff so a fresh agent can resume the work with zero context loss.\n\nArguments / focus hints provided by the user: {{args}}\n\nFollow the handoff workflow: reconstruct the objective and Definition of Done; load persisted state (.beads/plans/active-plan.md, .beads/context/*, bare `bd prime` with project-specific recovery context defined by the tracked `.beads/PRIME.md` override); establish current status and the real working-tree state (git status, recent commits, uncommitted/unpushed changes); identify all required reading (specs, designs, plans, code file:line anchors, tests) with why each matters; write a self-contained document to docs/handoffs/handoff-<YYYY-MM-DD-HHmm>.md; then emit exactly one closing sentence: `Read docs/handoffs/handoff-<...>.md and do <concrete next action>.`'
  },
  'pr-shepherd': {
